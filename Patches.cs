...
internal static class Patches
{
    ...
    private static void ApplyAdaptiveLayout(PlanScreen screen)
    {
        ...
        var maxColumns = Mathf.Clamp(ModAssets.Options.MaxColumns, baseColumns, 12);
        if (maxColumns < baseColumns) maxColumns = baseColumns;

        var targetMaxRows = GetMaxVisibleRows(screen);
        ChooseColumnCounts(groups, baseColumns, maxColumns, targetMaxRows);

        var totalActualRows = GetTotalRows(groups);
        ...
        ResizePanel(screen, groups, width, visibleRowsForPanel);
    ...
    private static int GetMaxVisibleRows(PlanScreen screen)
    {
        ...
        var configMaxRows = ModAssets.Options?.MaxRows ?? 8;
        configMaxRows = Mathf.Max(1, configMaxRows);
